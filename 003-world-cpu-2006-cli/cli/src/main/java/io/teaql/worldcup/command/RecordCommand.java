...
public class RecordCommand implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (ht != null && ht.getTeamName() == null) {
                ht = Q.tournamentTeams().withIdIs(ht.getId()).comment("Load home team").purpose("Display").executeForList(ctx).first();
                match.updateHomeTeam(ht);
            }
            if (at != null && at.getTeamName() == null) {
                at = Q.tournamentTeams().withIdIs(at.getId()).comment("Load away team").purpose("Display").executeForList(ctx).first();
                match.updateAwayTeam(at);
            }
    ...
    private void recalculateGroupStandings(UserContext ctx, String groupLetter) throws Exception {
        ...
        for (GroupStanding standing : sortedStandings) {
            int newRank = rank++;
            if (!java.util.Objects.equals(standing.getStandingRank(), newRank)) {
                standing.updateStandingRank(newRank);

...
import io.teaql.core.oracle.OracleDataServiceExecutor
...
class OracleIntegrationTest {
    ...
    companion object {
        ...
        @BeforeAll
        fun setupDatabase() {
            println("Connecting to local Oracle XE...")
            val baseUrl = "jdbc:oracle:thin:@localhost:1521/XEPDB1"
            val baseUrlAlt = "jdbc:oracle:thin:@localhost:1521:XE"
            val user = "SYSTEM"
            val passwordsToTry = listOf("oracle", "123abc!@#", "password", "SYS", "admin")

            println("Initializing TeaQLManager with Oracle...")
            ...
            if (!connected) throw RuntimeException("Could not connect to Oracle with any common password")
            ...
            val dataService = OracleDataServiceExecutor("default", adapter)
